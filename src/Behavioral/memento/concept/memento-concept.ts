...
class Originator {
  ...
  public get memento() {
    ...
    return new Memento(this.#state).state;
  }

  public set memento(value: string) {
    this.#state = value;
...
class CareTaker {
  ...
  create(): void {
    ...
    const memento = this.#originator.state;
    this.#mementos.push(new Memento(memento));
  }
  ...
  restore(idx: number): void {
    ...
    const memento = this.#mementos[idx];
    this.#originator.memento = memento.state;
  }
}
